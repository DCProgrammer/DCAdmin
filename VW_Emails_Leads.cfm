...
<!----
This page checks all new emails agains members and certificate table to verify if
a member has already generated a referral to our dealers or partner dealers.
If they have, this page will output all user emails and the editor must mark/check
the Lead Generated Table to remove them from the queue

This application should only run after a Duplicate check for emails has been initiated.
--->
...
and 
email in (select email from Members a join certificat b on a.memberid=b.member join affiliate_table c on b.organizati = c.orgid
...
<tr bgcolor=#RowColor#>
	...
	<td align="center"><form><font size="-1"><input type="Submit" value="UPDATE"></font></form></td>
</tr>
</cfoutput>
</cfif>
