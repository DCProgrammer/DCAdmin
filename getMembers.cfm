...
<cfelseif FORM.membercsv EQ "Yes">
	<cfquery name="getMembers" datasource="#Application.MainDB#" username="#Application.DBUser#" password="#Application.DBPwd#">
		SELECT     Org, newMemberID, rtrim(FirstName) as firstname, rtrim(LastName) as lastname,
		rtrim(Addr1) as Addr1, rtrim(City) as City, rtrim(State) as State, rtrim(Zip) as Zip,
		rtrim(HomePhone) as HomePhone, rtrim(BPhone) as BPhone, rtrim(Email) as Email,
		JoinDate, rtrim(WorkFax) as WorkFax, rtrim(Fax) as Fax, rtrim(CellPhone) as CellPhone
		FROM         Members
		WHERE 1 = 1
		<cfif FORM.hasEmail EQ "Yes">
			AND (Email <> '')
	...
	<CFCONTENT type="text/csv">
		...
		<!--- Output Column Headers --->
		<cfoutput>Org#chr(delim)#NewMemberID#chr(delim)#FirstName#chr(delim)#LastName#chr(delim)#Addr1#chr(delim)#City#chr(delim)#State#chr(delim)##chr(delim)#HomePhone#chr(delim)#BPhone#chr(delim)#Email#chr(delim)#JoinDate#chr(delim)#Work Fax#chr(delim)#Fax#chr(delim)#CellPhone</cfoutput>

		<cfoutput>#chr(13)#</cfoutput> <!--- line break after column header --->

		<!--- Spill out data from a query --->
	<cfoutput query="getmembers">#getMembers.Org##chr(delim)##getMembers.NewMemberID##chr(delim)##getMembers.FirstName##chr(delim)##getMembers.LastName##chr(delim)##getMembers.Addr1##chr(delim)##getMembers.City##chr(delim)##getMembers.State##chr(delim)##getMembers.Zip##chr(delim)##getMembers.HomePhone##chr(delim)##getMembers.BPhone##chr(delim)##getMembers.Email##chr(delim)##getMembers.JoinDate##chr(delim)##getMembers.WorkFax##chr(delim)##getMembers.Fax##chr(delim)##getMembers.CellPhone##chr(13)#</cfoutput>
...
<cfelse>
	...
	<form action="GetMembers.cfm" name="getmembers" method="post">
