...
<cfif not isDefined("Organizati")>
	Page is not available
	<cflocation url="Certificate_Search.cfm">
</cfif>
<cfinclude template="_header.cfm">

<cfset PassedOrg='#URL.Organizati#'>

<cfquery name="GetCert" datasource="MainSQL" username="#Application.DBUser#" password="#Application.DBPwd#">
SELECT Distinct 
	A.Certificat, A.ADD_Date, A.Paid as CertificatPaid,
        B.DealerID, B.Name as DealerName, B.Addr1, B.City, B.State, B.Zip, B.Phone, 
        B.Contact, B.SalesRep as DCSalesRep,
	C.FirstName, C.LastName, C.Addr1 as MAddress, C.City as MCity, C.State as MState, 
        C.Zip as MZip, C.HomePhone, C.BPhone, C.CellPhone,
	D.Name as OrgName,
	E.Product,
	F.Paid as InvPaid
FROM Certificat A, DCDealers B, MembersTable C, Organiza D, Product E, INV_LINE F
WHERE A.ADD_DATE > '#Session.Add_Date#'
    and A.Organizati='#Trim(PassedOrg)#'
...
<tr bgcolor="#445999">
	<td class="HeadWhite" colspan="2">
	TOTAL: <cfoutput>#GetCert.RecordCount# FOR #GetCert.OrgName# FOR DATE OF #UCase(LSDateFormat(Session.Add_Date,'MMMM DD, YYYY'))# </cfoutput>
	</td>
</tr>
<tr bgcolor="#445999">
